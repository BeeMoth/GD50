    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                self.prevMovedTile = self.board.tiles[y][x]

                self.highlightedTile.gridX = self.prevMovedTile.gridX
                self.highlightedTile.gridY = self.prevMovedTile.gridY
                self.prevMovedTile.gridX = tempX
                self.prevMovedTile.gridY = tempY

                -- swap tiles in the tiles table
                self.board.tiles[self.highlightedTile.gridY][self.highlightedTile.gridX] =
                    self.highlightedTile

                self.board.tiles[self.prevMovedTile.gridY][self.prevMovedTile.gridX] = self.prevMovedTile

                -- tween coordinates between the two so they swap
                Timer.tween(0.1, {
                    [self.highlightedTile] = {x = self.prevMovedTile.x, y = self.prevMovedTile.y},
                    [self.prevMovedTile] = {x = self.highlightedTile.x, y = self.highlightedTile.y}
                })
                
                -- once the swap is finished, we can tween falling blocks as needed
                :finish(function()
                    local calMatches = self:calculateMatches()
					--if there isn't a match, play an error noise and reset the two block's positions
					if calMatches < 1 then
						gSounds['error']:play()
						
						self.highlightedTile = self.prevHTile
...
function PlayState:calculateMatches()
	self.prevHTile = self.highlightedTile
    self.highlightedTile = nil
    ...
    if matches then
        ...
        local varietyMultiplier = self.board:removeMatches()
		
		self.score = self.score + varietyMultiplier * 100
